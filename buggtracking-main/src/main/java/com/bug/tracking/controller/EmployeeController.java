...
public class EmployeeController {
	...
	@PostMapping("add")
	public ResponseEntity<?> createNewEmployee(@Valid @RequestBody Employee bugEntity,BindingResult result) {
		ResponseEntity<?> errorMap=errorMapValidationService.mapValidationError(result);
		if(errorMap!=null) return errorMap;
		Employee savedEmployee = employeeService.saveOrUpdate(bugEntity);
		return new ResponseEntity<>(savedEmployee,HttpStatus.CREATED);
	}
	@GetMapping("view/{id}")
	public ResponseEntity<?> getEmployeeById(@PathVariable Long id){
			Employee uugEntity=employeeService.findEmployeeById(id);
		return new ResponseEntity<>(uugEntity,HttpStatus.OK);
		}
	@GetMapping("viewall")
